 ...
 *
 * Design principle: pure facts only — NO display text, NO i18n keys.
 * Consumers map to their own text:
 *  - innsyn-api: maps to HendelseTekstType / i18n keys for the frontend
 *  - modia-api: maps to Norwegian strings in Titler.kt
 *
 * The nullable boolean fields are load-bearing:
 *  - [SoknadsStatusEndret.mottakerNavn] lets innsyn reconstruct SOKNAD_MOTTATT_MED/UTEN_KOMMUNENAVN
 *  - [TildeltNavKontor.enhetNavnOppslagFeilet] and [erForsteTildeling] reconstruct the four
 *    SOKNAD_VIDERESENDT_* variants together with [Soknad.erPapirsoknad]
 */
...
data class SoknadsStatusEndret(
    ...
    /**
     * The name of the Nav-enhet that received the application, if known at the time
     * the MOTTATT hendelse was processed. null means the name was not available.
...
data class TildeltNavKontor(
    ...
    /** true = NORG lookup failed; enhet navn is synthetic ("et annet Nav-kontor"). */
...
/** Fired when a DokumentasjonEtterspurt empties the oppgave-list (status != BEHANDLES_IKKE). */
